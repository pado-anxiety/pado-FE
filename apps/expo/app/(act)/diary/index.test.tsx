...
jest.mock('@src/lib/route', () => ({
  ROUTES: { HOME: '/', ACT: { DIARY: { STEP: '/(act)/diary/step' } } },
  WEBVIEW_ROUTES: { ACT: { DIARY: { BASE: '/act/diary' } } },
...
describe('DiaryScreen (Intro)', () => {
  ...
  it('NEXT 액션 수신 시 Step 화면으로 이동한다', () => {
    ...
    expect(mockPush).toHaveBeenCalledWith('/(act)/diary/step');
  });
