...
import ActionScreen from './index';
...
jest.mock('@src/lib/route', () => ({
  ROUTES: { HOME: '/', ACT: { ACTION: { STEP: '/(act)/action/step' } } },
  WEBVIEW_ROUTES: { ACT: { ACTION: { BASE: '/act/action' } } },
...
describe('ActionScreen (Intro)', () => {
  ...
  it('NEXT 액션 수신 시 Step 화면으로 이동한다', () => {
    ...
    expect(mockPush).toHaveBeenCalledWith('/(act)/action/step');
  });
